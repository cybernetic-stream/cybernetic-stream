...
app.post('/', async (req, res) => {
	const { videoId, docId } = JSON.parse(Buffer.from(req.body.message.data, 'base64').toString('utf-8'));
...
async function getStreamURLS(id) {
	...
	return new Promise((resolve, reject) => {
		const YT_DLP_ARGS = ['-f', 'bestvideo[ext=webm]+bestaudio[ext=webm]/best[ext=webm]', '-g'];
